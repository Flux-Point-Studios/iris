...
export class SplashPreprodAnalyzer extends BaseAmmDexAnalyzer {
    ...
    protected async swapOrders(transaction: Transaction): Promise<LiquidityPoolSwap[]> {
        return transaction.outputs.map((output: Utxo) => {
            ...
            try {
                ...
                const swapInToken: Token = datumParameters.SwapInTokenPolicyId === ''
                    ? 'lovelace'
                    : new Asset(datumParameters.SwapInTokenPolicyId as string, datumParameters.SwapInTokenAssetName as string);
                const swapOutToken: Token = datumParameters.SwapOutTokenPolicyId === ''
                    ? 'lovelace'
    ...
    protected liquidityPoolStates(transaction: Transaction): LiquidityPoolState[] {
        return transaction.outputs.map((output: Utxo) => {
            ...
            if ([SPECTRUM_POOL_V1_CONTRACT_ADDRESS, SPECTRUM_POOL_V2_CONTRACT_ADDRESS].includes(output.toAddress)) {
                return undefined;
            }
            ...
            try {
                ...
                const nft: Asset | undefined = output.assetBalances.find((balance: AssetBalance) => {
                    return balance.asset.identifier() === `${datumParameters.TokenPolicyId}${datumParameters.TokenAssetName}`;
                })?.asset;
                const lpTokenBalance: AssetBalance | undefined = output.assetBalances.find((balance: AssetBalance) => {
                    return balance.asset.identifier() === `${datumParameters.LpTokenPolicyId}${datumParameters.LpTokenAssetName}`;
                });
